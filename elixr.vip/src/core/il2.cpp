...
namespace il2 {
	void init()
	{
		...
		using cpp_domain_get = uintptr_t(*)();
		methods::domain_get = LI_FIND_DEF(cpp_domain_get);

		using cpp_class_get_methods = uintptr_t(*)(uintptr_t, uintptr_t*);
		methods::class_get_methods = LI_FIND_DEF(cpp_class_get_methods);

		using cpp_method_get_param = uintptr_t(*)(uintptr_t, int);
		methods::method_get_param = LI_FIND_DEF(cpp_method_get_param);

		using cpp_method_get_param_count = int (*)(uintptr_t);
		methods::method_get_param_count = LI_FIND_DEF(cpp_method_get_param_count);

		using cpp_assembly_get_image = uintptr_t(*)(uintptr_t);
		methods::assembly_get_image = LI_FIND_DEF(cpp_assembly_get_image);

		using cpp_domain_get_assemblies = uintptr_t * (*)(void*, uintptr_t*);
		methods::domain_get_assemblies = LI_FIND_DEF(cpp_domain_get_assemblies);

		using cpp_class_from_name = uintptr_t(*)(uintptr_t, const char*, const char*);
		methods::class_from_name = LI_FIND_DEF(cpp_class_from_name);

		using cpp_resolve_icall = uintptr_t(*)(const char*);
		methods::resolve_icall = LI_FIND_DEF(cpp_resolve_icall);

		using cpp_class_get_field_from_name = uintptr_t(*)(uintptr_t, const char*);
		methods::class_get_field_from_name = LI_FIND_DEF(cpp_class_get_field_from_name);

		using cpp_field_static_get_value = uintptr_t(*)(uintptr_t, uintptr_t*);
		methods::field_static_get_value = LI_FIND_DEF(cpp_field_static_get_value);

		using cpp_class_get_fields = uintptr_t(*)(uintptr_t, uintptr_t*);
		methods::class_get_fields = LI_FIND_DEF(cpp_class_get_fields);

		using cpp_field_get_offset = uintptr_t(*)(uintptr_t);
		methods::field_get_offset = LI_FIND_DEF(cpp_field_get_offset);

		using cpp_object_new = uintptr_t(*)(uintptr_t);
		methods::object_new = LI_FIND_DEF(cpp_object_new);

		using cpp_type_get_object = uintptr_t(*)(uintptr_t);
		methods::type_get_object = LI_FIND_DEF(cpp_type_get_object);

		using cpp_class_get_type = uintptr_t(*)(uintptr_t);
		methods::class_get_type = LI_FIND_DEF(cpp_class_get_type);

		using cpp_runtime_class_init = uintptr_t(*)(uintptr_t);
		methods::runtime_class_init = LI_FIND_DEF(cpp_runtime_class_init);

		using cpp_string_new_wrapper = il2cppstring * (*)(const char*);
		methods::new_string = LI_FIND_DEF(cpp_string_new_wrapper);
	}
	...
	{
		uintptr_t il2_method = method(kl.c_str(), name.c_str(), arg, name_space);
		*reinterpret_cast<void**>(il2_method) = our_func;

		void* written = *reinterpret_cast<void**>(il2_method);
		if (!written)
			return NULL;
	...
	uintptr_t value(const char* kl, const char* name, bool get_offset, const char* name_space)
	{
		...
		{
			uintptr_t out = 0;
			uintptr_t cpp_field = NULL;
			uintptr_t field_offset = NULL;

			while (cpp_field = methods::class_get_fields(this_class, &out))
			{
				char* char_name = (char*)*reinterpret_cast<uintptr_t*>(cpp_field);
				if (!char_name)
					continue;

				uintptr_t offset = methods::field_get_offset(cpp_field);
